/*
	...
	Run:
	lambda-local -l index.js -t 9000 -e event-lab.json
	lambda-local -l index.js -t 9000 -e event-verify-signed-data-lab.json
	lambda-local -l index.js -t 9000 -e event-verify-signed-data-policy-lab.json
	lambda-local -l index.js -t 9000 -e event-generate-wallet-lab.json
	lambda-local -l index.js -t 9000 -e event-generate-hash-lab.json

	lambda-local -l index.js -t 9000 -e event-generate-account-lab.json
	lambda-local -l index.js -t 9000 -e event-generate-transaction-lab.json

	lambda-local -l index.js -t 9000 -e event-verify-user-auth-lab.json
	lambda-local -l index.js -t 9000 -e event-util-keys.json

	lambda-local -l index.js -t 9000 -e event-verify-signed-data-policy-internal-lab.json
			...
			*/
			...
			entityos.add(
			{
				...
				code: function ()
				{
					...
					if (request.body.apikey != undefined)
					{
						...
						{
							if (request.body.method == 'verify-user-auth')
							{
				...
				code: function ()
				{
					...
					*/
					...
					_addresses.base = Cardano.BaseAddress.new(
						...
						Cardano.StakeCredential.from_keyhash(_keys.public.to_raw_key().hash()),
						Cardano.StakeCredential.from_keyhash(_keys.stake.to_raw_key().hash()),
					);
			...
			entityos.add(
			{
				...
				code: function (param, response)
				{
					...
					entityos.invoke('app-process-generate-transaction-check');
				}
			});

			entityos.add(
			{
				name: 'app-process-generate-transaction-check',
				code: function (param, response)
				{
					let request = entityos.get({scope: '_request'});
					var data = _.get(request, 'body.data');

					const keys = 
					{
						user: data.userkey,
						conversation: data.conversationkey // Not used at this point, but may be as POST to transfer information for transaction.
					}
					...
					if (response == undefined)
					{
						...
						{
							entityos.invoke('util-end', {error: 'Missing User Key'}, '400');
						}
						else
						{
							entityos.cloud.search(
							{
								...
								callback: 'app-process-generate-transaction-check'
							});
						}
					}
					else
					{
						if (response.data.rows.length == 0)
						{
							entityos.invoke('util-end', {error: 'Bad User Key'}, '401');
						}
						else
						{
							let event = entityos.get({scope: '_event'});
							event._user = _.first(response.data.rows);
							entityos.set({scope: '_event', value: event});
							entityos.invoke('app-process-generate-transaction-process');
						}
					}
				}
			});
					...
					if (response == undefined)
					{
						...
						else
						{
							entityos.cloud.search(
							{
								...
								callback: 'app-process-generate-transaction-process'
							});
						}
					}
					else
